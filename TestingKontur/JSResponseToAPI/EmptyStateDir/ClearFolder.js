const myHeaders = new Headers();
myHeaders.append("Authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..UDe0wRKSbX7X4nPnmyB8ZHkK6bjiXBCiVTet4RVV_yM");

const getResponce = {
    method: "GET",
    headers: myHeaders,
    redirect: "follow"
};

const deleteResponce = {
    method: "DELETE",
    headers: myHeaders,
    redirect: "follow"
};

fetch("https://staff-testing.testkontur.ru/api/v1/cfs/dir/e0e8b17f-81ae-4392-8b1f-61d6d2cb8319", getResponce)
    .then((response) => response.text())
    .then((result) => {
        let data = JSON.parse(result)
        let files = data.content.files
        files.forEach((file) => {
            fetch("https://staff-testing.testkontur.ru/api/v1/cfs/files/" + file.id, deleteResponce).then((response) => console.log("file delete " + file.id));
        });
    })
    .catch((error) => console.error(error));
